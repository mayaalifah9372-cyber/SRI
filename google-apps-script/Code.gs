 ...
 *                         (DB_PESANAN, DB_PRODUK_JASA, DB_DOKUMEN_DRIVE, DB_PENGGUNA, DB_PENGATURAN, DB_LOG_AKTIVITAS)
...
function doPost(e) {
  try {
    ...
    if (action === "login") {
      ...
      if (userFound) {
        // Update waktu login terakhir di spreadsheet
        if (rowIndex > 1) {
    ...
    if (action === "register") {
      ...
      // Cek apakah username atau no whatsapp sudah ada
      var data = sheetUsers.getDataRange().getValues();
